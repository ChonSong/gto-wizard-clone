...
## Active Tasks (2026-06-25 Coach Cycle)

> Generated by Coach after review cycle. Previous recovery tasks (fix-matrix-legend, fix-strategies-placeholders, fix-solutions-text, fix-sb-range-fallback, fix-matrix-legend-call-color) are all completed and deployed. These are the remaining open spec_gaps.

### Task: fix-filters-sub-tab-stub
- **Description**: The Filters sub-tab in the study page right sidebar shows "Hand/blocker filters coming soon" placeholder text with no functional controls. Implement hand filter controls (select/deselect hands by category) and blocker filter inputs (select blocker cards to show removal effects). The reference interface shows filterable hand lists and blocker card selectors.
- **Location**: `apps/web/src/app/study/page.tsx` — right sub-tab rendering for 'filters' tab
- **Success criteria**:
  - Filters sub-tab shows functional hand category filters (e.g., pairs, suited, offsuit checkboxes)
  - Blocker inputs allow selecting specific cards to show card removal effect
  - Filter state updates the hand matrix and range display in real time
  - No console errors
  - 53/53 frontend tests pass, 368/368 Python tests pass
- **Coach checks**: Load /study, switch to right sidebar Filters sub-tab, verify filter controls exist and function. Toggle a category filter and verify matrix updates.

### Task: fix-blockers-sub-tab-missing
- **Description**: The right sidebar has Overview, Table, and Equity chart tabs, but no Blockers sub-tab under Overview. The interaction spec lists Blockers as a sub-tab that should show card removal analysis (which blocking cards are in opponent's range, which are removed). Add the Blockers sub-tab with card removal analysis display.
- **Location**: `apps/web/src/app/study/page.tsx` — right sidebar sub-tab navigation and content
- **Success criteria**:
  - Blockers sub-tab appears in the right sidebar sub-tab row (Hand, Summary, Filters, Actions, Blockers)
  - Card removal analysis shows which cards block/unblock opponent range
  - Data sourced from solver API range data
  - No console errors
- **Coach checks**: Load /study, click Blockers sub-tab, verify card removal analysis renders.

### Task: fix-spaced-repetition-practice-mode
- **Description**: The /practice page Spaced Repetition mode selector shows "coming soon" notice. Implement the SR-2 algorithm training mode: per-spot easeFactor/interval tracking, localStorage persistence, due-for-review count in session setup, next review time display, and session summary with due-now statistics.
- **Location**: `apps/web/src/app/practice/page.tsx` — mode selector and SR implementation
- **Success criteria**:
  - Spaced Repetition mode is selectable in practice setup
  - SR algorithm tracks ease factor, interval, next review date per spot
  - localStorage persists SR state between sessions
