...
## Active Tasks (2026-06-26 Coach Cycle)

> All tasks from previous cycle are COMPLETED and verified live. No open spec_gaps remain. Coach approval batch for 6 pending tasks: fix-filters-sub-tab-stub (8d54020), fix-blockers-sub-tab-missing (ac3a20b), fix-spaced-repetition-practice-mode (2768fed), fix-position-selector-player-tiles (eff98bb), fix-e2e-test-runner-conflict (b07dccd), fix-top-bar-action-prompts (59124ac). All verified live at wiz.codeovertcp.com — right sidebar has all 9 sub-tabs functional, player tiles with stack bb, action prompt header row, Spaced Repetition mode working, E2E test runner fixed. Next phase TBD — awaiting Coach backlog generation.

### Task: tbd
- **Description**: No remaining Phase 2 spec gaps. All 9 right sidebar sub-tabs (HAND, SUMMARY, FILTERS, ACTIONS, ACT CHART, RANGE CMP, BLOCKERS, EQ CHART, CMP EV) functional. Position selector uses player tiles with stack bb. Action prompt header row present. Spaced Repetition mode functional. E2E test runner resolved. Next step: Coach to evaluate Phase 3 scope.
- **Success criteria**: TBD — awaiting Coach backlog generation.
- **Coach checks**: N/A
...
### Task: fix-matrix-legend-call-color ✅ Already implemented (lines 1295-1299 show tri-color legend; ACTION_COLORS includes 'call': BLUE at line 75)
...
### Task: fix-suit-colors-standard-poker ✅ Already implemented (line 2116: suitColors = {s: '#fff', h: '#E53935', d: '#E53935', c: '#fff'})
